...
void phiRatio(){
	...
	for ( int p=0; p<nentries ;p++){
		...
		//		//global
		if (charge[0][1] > 0) {
			phihistoGlbUP->Fill(unprop_phi[0][1]);
		}
		if (charge[0][1] < 0) {
		...
		//Tpfms
		if (charge[3][1] > 0) {
			phihistoTpfmsUP->Fill(unprop_phi[3][1]);
		}
		if (charge[3][1] < 0) {
		...
		//Picky
		if (charge[4][1] > 0) {
			phihistoPickyUP->Fill(unprop_phi[4][1]);
		}
		if (charge[4][1] < 0) {
		...
		//DYT
		if (charge[5][1] > 0) {
			phihistoDYTUP->Fill(unprop_phi[5][1]);
		}
		if (charge[5][1] < 0) {
		...
		//TuneP
		if (charge[8][1] > 0) {
			phihistoTunePUP->Fill(unprop_phi[8][1]);
		}
		if (charge[8][1] < 0) {
	...
	leg5->AddEntry(phihistoTunePUP,"#mu^{+} ","l");
	leg5->AddEntry(phihistoTunePLOW,"#mu^{-}","l");
	leg5 -> Draw();
	...
	// h1 settings
	phihistoTunePUP->SetXTitle("#phi-TuneP lower leg tag ");
	...
	leg0->AddEntry(phihistoGlbUP,"#mu^{+} ","l");
	leg0->AddEntry(phihistoGlbLOW,"#mu^{-}","l");
	leg0 -> Draw();
	...
	leg1->AddEntry(phihistoTkonlyUP,"#mu^{+} ","l");
	leg1->AddEntry(phihistoTkonlyLOW,"#mu^{-}","l");
	leg1 -> Draw();
	...
	leg2->AddEntry(phihistoTpfmsUP,"#mu^{+} ","l");
	leg2->AddEntry(phihistoTpfmsLOW,"#mu^{-}","l");
	leg2 -> Draw();
	...
	leg3->AddEntry(phihistoPickyUP,"#mu^{+} ","l");
	leg3->AddEntry(phihistoPickyLOW,"#mu^{-}","l");
	leg3 -> Draw();
	...
	leg4->AddEntry(phihistoDYTUP,"#mu^{+} ","l");
	leg4->AddEntry(phihistoDYTLOW,"#mu^{-}","l");
	leg4 -> Draw();
