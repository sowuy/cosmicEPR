...
void pTRatio(){
	...
	for ( int p=0; p<nentries ;p++){
		...
		//		//global
		if (charge[0][1] > 0) {
			pthistoGlbUP->Fill(unprop_pt[0][1]);
		}
		if (charge[0][1] < 0) {
		...
		//Tkonly
		if (charge[2][1] > 0) {
			pthistoTkonlyUP->Fill(unprop_pt[2][1]);
		}
		if (charge[2][1] < 0) {
		...
		//Tpfms
		if (charge[3][1] > 0) {
			pthistoTpfmsUP->Fill(unprop_pt[3][1]);
		}
		if (charge[3][1] < 0) {
		...
		//Picky
		if (charge[4][1] > 0) {
			pthistoPickyUP->Fill(unprop_pt[4][1]);
		}
		if (charge[4][1] < 0) {
		...
		//DYT
		if (charge[5][1] > 0) {
			pthistoDYTUP->Fill(unprop_pt[5][1]);
		}
		if (charge[5][1] < 0) {
		...
		//TuneP
		if (charge[8][1] > 0) {
			pthistoTunePUP->Fill(unprop_pt[8][1]);
		}
		if (charge[8][1] < 0) {
	...
	// h1 settings
	pthistoTunePUP->SetXTitle("p_{t}-TuneP lower leg tag");
	...
	leg0->AddEntry(pthistoGlbUP,"#mu^{+} ","l");
	leg0->AddEntry(pthistoGlbLOW,"#mu^{-}","l");
	leg0 -> Draw();
	...
	leg1->AddEntry(pthistoTkonlyUP,"#mu^{+} ","l");
	leg1->AddEntry(pthistoTkonlyLOW,"#mu^{-}","l");
	leg1 -> Draw();
	...
	leg2->AddEntry(pthistoTpfmsUP,"#mu^{+} ","l");
	leg2->AddEntry(pthistoTpfmsLOW,"#mu^{-}","l");
	leg2 -> Draw();
	...
	leg3->AddEntry(pthistoPickyUP,"#mu^{+} ","l");
	leg3->AddEntry(pthistoPickyLOW,"#mu^{-}","l");
	leg3 -> Draw();
	...
	leg4->AddEntry(pthistoDYTUP,"#mu^{+} ","l");
	leg4->AddEntry(pthistoDYTLOW,"#mu^{-}","l");
	leg4 -> Draw();
