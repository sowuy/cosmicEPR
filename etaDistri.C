...
void etaDistri(){
	...
	for ( int p=0; p<nentries ;p++){
		...
		if (pixel_hits[8][0]>=1 && strip_layers[8][0]>=5 && pixel_hits[8][1]>=1 && strip_layers[8][1]>=5  && unprop_pt[8][1]>30.){
			...
			//Pt Bin
			//if (unprop_pt[8][1]>100 && unprop_pt[8][1]<500){

			//		//global
			/*etahistoGlbUP->Fill(eta[0][0]);
			  etahistoGlbLOW->Fill(eta[0][1]);
			//Tkonly
			etahistoTkonlyUP->Fill(eta[2][0]);
			etahistoTkonlyLOW->Fill(eta[2][1]);
			//Tpfms
			etahistoTpfmsUP->Fill(eta[3][0]);
			etahistoTpfmsLOW->Fill(eta[3][1]);
			//Picky
			etahistoPickyUP->Fill(eta[4][0]);
			etahistoPickyLOW->Fill(eta[4][1]);
			//DYT
			etahistoDYTUP->Fill(eta[5][0]);
	...
	// this is the root file you have input with your tree inside
	MCtreedata->Add("ROOT_Files/MC2016/MC_2016_P100.root");

	// the following lines are to extract the branch of the trees, when there is a & it means that there is only one entry per event, like for run, lumi, event, dileetaon mass in my case for DY events.
	MCtreedata->SetBranchAddress("charge",charge);
	//MCtreedata->SetBranchAddress("theta",theta);
	MCtreedata->SetBranchAddress("unprop_charge",unprop_charge);
	MCtreedata->SetBranchAddress("unprop_theta",unprop_theta);
	...
	Long64_t MCnentries = MCtreedata->GetEntries();


	for ( int p=0; p<MCnentries ;p++){
	// this loop over all the events in your tree

	MCtreedata->GetEntry(p);
	//apply some quality cuts to select the entries that we think are interesting
	//
	//
	//Algorithm choice
	...
	Double_t norm = 1;
	Double_t scale_DATA = norm/(etahistoTunePUP->Integral());
	etahistoTunePUP->Scale(scale_DATA);
	Double_t scale_MC = norm/(etahistoTunePLOW->Integral());
	etahistoTunePLOW->Scale(scale_MC);*/
	...
	//etahistoTunePUP->SetTitle("TuneP-2016+2017 data-P500");
	...
	leg5->AddEntry(etahistoTunePUP,"Upper leg ","l");
	leg5->AddEntry(etahistoTunePLOW,"Lower leg","l");
	leg5 -> Draw();
	...
	// h1 settings
	etahistoTunePUP->SetXTitle("#eta-TuneP ");
	...
	leg0->AddEntry(etahistoGlbUP,"Upper leg ","l");
	leg0->AddEntry(etahistoGlbLOW,"Lower leg","l");
	leg0 -> Draw();
	...
	leg1->AddEntry(etahistoTkonlyUP,"Upper leg ","l");
	leg1->AddEntry(etahistoTkonlyLOW,"Lower leg","l");
	leg1 -> Draw();
	...
	leg2->AddEntry(etahistoTpfmsUP,"Upper leg ","l");
	leg2->AddEntry(etahistoTpfmsLOW,"Lower leg","l");
	leg2 -> Draw();
	...
	leg3->AddEntry(etahistoPickyUP,"Upper leg ","l");
	leg3->AddEntry(etahistoPickyLOW,"Lower leg","l");
	leg3 -> Draw();
	...
	leg4->AddEntry(etahistoDYTUP,"Upper leg ","l");
	leg4->AddEntry(etahistoDYTLOW,"Lower leg","l");
	leg4 -> Draw();
